...
function ChatCard(props) {
  const {index, user, onPress, isTyping, typing_id} = props;
  // const [lastmessage, lastMessage] = useState(user.lastMessageBody);
  const {username, unreadCount, friend_id, lastMessageBody} = user;
  ...
  const openChat = () => {
    onPress(friend_id, username, unreadCount);
  };
  return (
      ...
      <Card containerStyle={styles.card}>
        <View style={{flex: 1, flexDirection: 'row', justifyContent: 'center'}}>
          ...
          <View style={{flex: 5, flexDirection: 'column'}}>
            ...
            {isTyping.includes(friend_id) ? (
              <Text style={styles.typing}>Typing...</Text>
            ) : (
              <Text style={styles.message}>
                {lastMessageBody.slice(0, 100)}
              </Text>
            )}
          </View>
          <View
